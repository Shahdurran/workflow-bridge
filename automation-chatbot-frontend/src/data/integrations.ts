...
export const getAppIcon = (appName: string | undefined | null) => {
  if (!appName) {
    return appIconMap['unknown'] || Settings;
  }
  const normalizedName = appName.toLowerCase().replace(/[^a-z0-9-]/g, '-');
  return appIconMap[normalizedName] || appIconMap['unknown'] || Settings;
};

export const getAppDisplayName = (appName: string | undefined | null): string => {
  if (!appName) {
    return 'Unknown App';
  }
